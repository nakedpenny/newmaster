...
def get_page_info_list(comic_folder: str, comic_info: RawConfigParser, delete_scheduled_posts: bool) \
    ...
    for page_path in glob(f"your_content/{comic_folder}comics/*/"):
        ...
        if post_date > local_time:
            ...
            page_info["file_list"] = [filename] if filename else str_to_list(page_info.get("File list", ""))
            page_info["Volume"] = page_info.get("Volume", "")
            page_info["Storyline"] = page_info.get("Chapter", "")
...
def create_comic_data(comic_folder: str, comic_info: RawConfigParser, page_info: dict,
    ...
    comic_paths = [page_dir + f for f in page_info["file_list"]]
    ...
    return {
        ...
        "volume": page_info["Volume"],
        "storyline": page_info["Storyline"],
...
def process_comic_images(comic_info: RawConfigParser, comic_data_dicts: List[Dict]):
    ...
    if comic_info.getboolean(section, "Create thumbnails"):
        for comic_data in comic_data_dicts:
            process_comic_image(comic_info, comic_data["comic_paths"][0])
...
def get_storylines(comic_data_dicts: List[Dict]) -> List[Dict[str, List]]:
    ...
    for comic_data in comic_data_dicts:
        ...
        if volume:
            ...
            if storyline:
                if storyline not in volumes_dict[volume].keys():
                    volumes_dict[volume][storyline] = []
                volumes_dict[volume][storyline].append(comic_data.copy())
